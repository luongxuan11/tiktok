...
const InputForm = ({text, label, type, icon, setShowPassword, htmlFor, setShowPasswordRepeat, setPayload, payload, invalidFields, setInvalidFields}) => {
  ...
  return (
    <div className='input row-column'>
        ...
        <input type={type} value={payload} id={htmlFor} onFocus={() => setInvalidFields && setInvalidFields([])} placeholder={`${text}...`} onChange={(e) => setPayload(prev => ({...prev, [htmlFor]: e.target.value}))}/>
        {icon && <span  onClick={htmlFor === "password" ? handleShowPassword : handleShowPasswordRepeat} className='icon'>{icon}</span>}
